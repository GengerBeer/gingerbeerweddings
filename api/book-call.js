...
export default async function handler(req, res) {
  ...
  try {
    ...
    const params = new URLSearchParams({ name: name || "", email: email || "", phone: phone || "", wedding_date: call_date || "", package: `📞 Book a Call — ${call_time || ""}`, message: message ? `Call time: ${call_time}\n\n${message}` : `Call time: ${call_time}` });

    const response = await fetch(`${scriptUrl}?${params.toString()}`, { method: "GET", redirect: "follow" });
