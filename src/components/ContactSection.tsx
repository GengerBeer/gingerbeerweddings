...
type FormData = { name: string; email: string; wedding_date: string; message: string; };
const initialForm: FormData = { name: "", email: "", wedding_date: "", message: "" };

const ServiceCheckbox = ({ item, checked, onChange }: { item: { id: string; label: string; desc: string }; checked: boolean; onChange: () => void; }) => (
  <label className={`flex items-center gap-4 cursor-pointer rounded-xl border px-4 py-3 transition-all duration-200 select-none ${checked ? "border-brand-sand/60 bg-brand-sand/10" : "border-brand-cream/10 hover:border-brand-sand/30"}`}>
    <input type="checkbox" checked={checked} onChange={onChange} className="sr-only" />
    <div className={`flex-shrink-0 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-200 pointer-events-none ${checked ? "border-brand-sand bg-brand-sand/20" : "border-brand-cream/30 bg-transparent"}`}>
      {checked && (<svg width="11" height="9" viewBox="0 0 11 9" fill="none"><path d="M1 4L4 7.5L10 1" stroke="hsl(34 89% 89%)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>)}
    </div>
...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const packageLabel = STEPS.flatMap((g) => g.items).filter((item) => selected.includes(item.id)).map((item) => item.label).join(", ") || "Not specified";
    try {
      const res = await fetch("/api/submit", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, package: packageLabel }) });
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 relative items-start">
          <div className="reveal">
            ...
            <h2 className="font-serif text-display-md text-brand-cream font-extrabold mb-12 md:mb-16 leading-tight">Let's Create Something<br /><em className="italic">Timeless</em></h2>
            <p className="font-sans text-brand-cream/55 text-sm leading-relaxed mb-12 max-w-sm">Whether you're a couple planning your wedding or a filmmaker looking for a dedicated post-production partner — we'd love to hear from you.</p>
          ...
          <div className="reveal reveal-delay-2" style={{ paddingTop: '3.5rem' }}>
            {submitted ? (
              <div className="h-full flex flex-col items-start justify-center gap-6 py-12">
                ...
                <h3 className="font-serif text-display-sm text-brand-cream font-extrabold">We've received your inquiry.</h3>
                <p className="font-sans text-brand-cream/55 text-sm leading-relaxed max-w-sm">Thank you for reaching out. We'll be in touch within 24 hours to discuss your project.</p>
                <button onClick={() => { setSubmitted(false); setForm(initialForm); setSelected([]); setStep(0); }} className="font-sans text-[11px] uppercase tracking-widest text-brand-sand border-b border-brand-sand/40 pb-0.5 hover:border-brand-sand transition-colors">Send Another Message</button>
              </div>
            ) : (
                ...
                {step >= 1 && step <= 3 && (
                  <div className="space-y-5">
                    ...
                    <div className="flex items-center gap-3 pt-1">
                      ...
                      <button type="button" onClick={handleNext} className="btn-primary flex items-center gap-2">{step === 3 ? "Next — Your Message" : `Next — ${STEPS[step].category}`}<ArrowRight size={13} /></button>
                    </div>
                  </div>
                )}
                {step === 4 && (
                  <div className="space-y-5">
                    <p className="font-sans text-[10px] uppercase tracking-widest text-brand-cream/50">Tell Us About Your Project</p>
                    <textarea name="message" placeholder="Style, special moments, anything you'd like us to know..." rows={5} value={form.message} onChange={handleChange} className="form-input resize-none" />
                    {selected.length > 0 && (<div className="rounded-xl border border-brand-cream/10 px-4 py-3 space-y-1"><p className="font-sans text-[9px] uppercase tracking-widest text-brand-cream/30">Selected services</p><p className="font-sans text-xs text-brand-cream/60 leading-relaxed">{STEPS.flatMap((g) => g.items).filter((item) => selected.includes(item.id)).map((item) => item.label).join(" · ")}</p></div>)}
                    <div className="flex items-center gap-3 pt-1">
                      <button type="button" onClick={handleBack} className="flex items-center gap-2 font-sans text-[11px] uppercase tracking-widest text-brand-cream/40 hover:text-brand-cream transition-colors"><ArrowLeft size={12} />Back</button>
                      <button type="submit" disabled={loading} className="btn-primary disabled:opacity-60 flex items-center gap-2">{loading ? (<><span className="w-3.5 h-3.5 rounded-full border-2 border-brand-burgundy/40 border-t-brand-burgundy animate-spin" />Sending...</>) : (<>Check Availability<ArrowRight size={13} /></>)}</button>
                    </div>
                  </div>
                )}
              </form>
